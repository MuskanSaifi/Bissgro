...
export default function EditBlog() {
  ...
  const fetchBlog = async () => {
    try {
      const res = await fetch(`/api/blogs/${params.id}`, { credentials: 'include' });
      ...
      if (res.ok && data.blog) {
        ...
        setImagePublicId(data.blog.imagePublicId || '');
        setContentImagePublicIds(data.blog.contentImagePublicIds || []);
        setMetaTitle(data.blog.metaTitle || '');
        setMetaDescription(data.blog.metaDescription || '');
        setMetaKeywords(data.blog.metaKeywords || '');
        setMetaImage(data.blog.metaImage || '');
        setCanonicalUrl(data.blog.canonicalUrl || '');
        setFocusKeyword(data.blog.focusKeyword || '');
        setRobots(data.blog.robots || 'index,follow');
      ...
      if (res.ok) {
        if (imagePublicId) {
          await fetch(`/api/blogs/${params.id}`, {
            ...
            body: JSON.stringify({ deleteOldImage: true }),
            credentials: 'include',
          });
        }
        setImage(data.url);
  ...
  const handleContentImageUploaded = (publicId) => {
    if (publicId) setContentImagePublicIds((prev) => [...prev, publicId]);
  };
  ...
  return (
    <>
      <Toaster />
      <div>
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '36px', marginBottom: '30px' }}>Edit Blog</h1>
