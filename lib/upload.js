...
/** Ensure upload directories exist so Next.js can serve /uploads/... and saveLocal can write.
 * Wrapped in try/catch: on serverless hosts (read-only filesystem) this must not crash the module. */
function ensureUploadDirs() {
  try {
    const dirs = [UPLOADS_DIR, path.join(UPLOADS_DIR, 'pages'), path.join(UPLOADS_DIR, 'blog'), path.join(UPLOADS_DIR, 'projects')];
    ...
    console.warn('Could not create upload dirs (read-only filesystem?):', err?.message);
  }
}
ensureUploadDirs();
...
export async function uploadImage(buffer, folder = 'blog', mimeType = 'image/jpeg') {
  try {
    ...
    // Fallback to local storage for any Cloudinary failure (missing config, 401, disabled, etc.)
    try {
      return saveLocal(buffer, folder, mimeType);
    } catch (localErr) {
      console.error('Local storage fallback also failed:', localErr?.message);
      // Surface the original Cloudinary error - it is the actionable one on live servers
      throw err;
    }
  }
}
